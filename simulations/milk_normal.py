...
def run_milk_normal(params: SimulationParams):
    """Молоко с нормальным распределением спроса и настраиваемым расписанием поставок"""
    ...
    SIGMA_BUYER = params.sigma_buyer
    total_days = params.days

    # Параметры нормального распределения для спроса
    DEMAND_MEAN = 25
    DEMAND_SIGMA = 1.41
    ...
    def calculate_buyer_percent_normal(expected, sigma):
        """Расчет процента покупателей по формуле с 12 числами"""
        r = [random.random() for _ in range(12)]
        deviation = sum(r) - 6
        percent = sigma * deviation + expected
        return max(0, min(100, percent))

    def generate_base_normal_demand():
        """Генерирует базовый спрос по нормальному закону"""
        demand = np.random.normal(DEMAND_MEAN, DEMAND_SIGMA)
        demand = max(20, min(30, demand))
        return demand
    ...
    for day in range(1, total_days + 1):
        ...
        # Проценты покупателей (нормальные через формулу)
        fifo_percent = calculate_buyer_percent_normal(FIFO_PERCENT, SIGMA_BUYER)
        lifo_percent = calculate_buyer_percent_normal(LIFO_PERCENT, SIGMA_BUYER)

        total = fifo_percent + lifo_percent
        fifo_percent = (fifo_percent / total) * 100
        lifo_percent = 100 - fifo_percent

        fifo_rates.append(fifo_percent)
        lifo_rates.append(lifo_percent)

        # СПРОС (нормальный) с учетом дня недели
        base_demand = generate_base_normal_demand()
        ...
        print(f"  РАСПРЕДЕЛЕНИЕ ПОКУПАТЕЛЕЙ: FIFO {fifo_percent:.1f}% / LIFO {lifo_percent:.1f}%")
        print(f"  Базовый спрос: {base_demand:.1f}, коэффициент: {weekday_factors[weekday]}, итого: {daily_demand}")
        ...
        if total_stock > 0 and daily_demand > 0:
            ...
            print(f"    FIFO ({fifo_percent:.1f}%): {fifo_count} покупателей")
            print(f"    LIFO ({lifo_percent:.1f}%): {lifo_count} покупателей")
